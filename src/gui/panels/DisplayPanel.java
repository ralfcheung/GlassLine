...
public class DisplayPanel extends JLayeredPane {
	 ...
	 */
	public DisplayPanel(FactoryPanel fPanel, Transducer linkedTransducer, ControlPanel cp) {
		parent = fPanel;
	 ...
	 */
	public void setupComponents(ControlPanel cp) {
		int numConveyors = 0;
		int numShuttles = 0;
		createBin(400,600);
		...
		createTransfer(currentComponent.getX()-(int)(GUIShuttle.leftConnector.getIconWidth()/2),currentComponent.getCenterY(),ConveyorDirections.UP, numShuttles++);
		...
		createTransfer(currentComponent.getCenterX(),currentComponent.getY()-(int)(GUIShuttle.leftConnector.getIconHeight()/2),ConveyorDirections.RIGHT, numShuttles++);
		...
		createTransfer(currentComponent.getX()+currentComponent.getIcon().getIconWidth()+(int)(GUIShuttle.leftConnector.getIconHeight()/2),currentComponent.getCenterY(),ConveyorDirections.DOWN, numShuttles++);
		...
		createTransfer(currentComponent.getCenterX(),currentComponent.getY()+currentComponent.getIcon().getIconHeight()+(int)(GUIShuttle.leftConnector.getIconHeight()/2),ConveyorDirections.LEFT, numShuttles++);
		...
		factory = new Factory(transducer, parent.getControlPanel().getTracePanel());
		
		cp.setLocation(280, 150);
		this.add(cp, CONTROL_PANEL_LAYER);
	}
	...
	{
		GUITruck truck = new GUITruck(transducer, x, y);
	...
	private void createTransfer(int x,int y,ConveyorDirections direction, int index)
	{
		GUIShuttle connector = new GUIShuttle(direction, true, index, transducer);
