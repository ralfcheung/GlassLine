...
public class GUISensor extends GuiComponent
{
	...
	public void checkGlassDetected()
	{
		...
		if((myIndex >= 0 && myIndex <= 3) || myIndex >= 26)
		{
			if (myIndex%2 == 0) {
			leftBound = 25;
			} 
		...
		if (pressed == false)
		{
			...
			for (int k = 0; k < this.activePieces.size(); k++)
			{
				...
				//
				if (activePieces.get(k).getCenterX()>=getCenterX()-leftBound&&activePieces.get(k).getCenterX()<=getCenterX()+rightBound&&activePieces.get(k).getCenterY()>=getCenterY()-lowerBound&&activePieces.get(k).getCenterY()<=getCenterY()+upperBound)
				{
					setIcon(sensorOn);
		...
		else
		{
			...
			if (currentGlassPiece.getCenterX()<getCenterX()-leftBound||currentGlassPiece.getCenterX()>getCenterX()+rightBound||currentGlassPiece.getCenterY()<getCenterY()-lowerBound||currentGlassPiece.getCenterY()>getCenterY()+upperBound)
			{
